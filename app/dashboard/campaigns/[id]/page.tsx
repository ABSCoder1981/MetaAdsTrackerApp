...
export default async function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: campaign } = await supabase
    .from("campaign")
    .select(
      "id, name, status, objective, meta_campaign_id, ad_account_id, ad_account(name, meta_ad_account_id, business_manager_id), property(name), sales_team_employee!campaign_manager_id_fkey(name)"
    )
    .eq("id", id)
    .single();
